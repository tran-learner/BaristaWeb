            ...
            @foreach ($ingredients as $ing)
                <div class="flex items-center justify-center gap-5">
                    ...
                    <input type="range" id="{{ $ing . 'Input' }}" name="coffee" min="0" max="1000"
                        value="500" oninput="updateValueCF(this.value)" class="sm:w-96 h-20">
                    <output id="{{ $ing . 'Output' }}">500</output>
                </div>
            @endforeach
            <div class="flex justify-center items-center w-full">
                <button class="bg-lightblue px-10 py-2 w-fit text-white font-extrabold rounded-md mt-5" id="submitBtn">OK</button>
            </div>
        </div>
    </div>
@endsection
